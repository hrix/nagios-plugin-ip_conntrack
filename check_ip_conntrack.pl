...
use vars qw($opt_c $opt_w
            $used_ip_conntrack $max_ip_conntrack
...
$used_ip_conntrack  = `sudo $command`;
...
$percent    = $used_ip_conntrack / $max_ip_conntrack * 100;
$fmt_pct    = sprintf "%.1f", $percent;
if ($percent >= $crit_level)
{
print "ip_conntrack CRITICAL - $fmt_pct% ($used_ip_conntrack) used\n";
exit $exit_codes{'CRITICAL'};
}
elsif ($percent >= $warn_level)
{
print "ip_conntrack WARNING - $fmt_pct% ($used_ip_conntrack) used\n";
exit $exit_codes{'WARNING'};
}
else
{
print "ip_conntrack OK - table usage = $fmt_pct%, count = $used_ip_conntrack";
exit $exit_codes{'OK'};
}
